...
three_days_ago=`mysql -u $db_user -e "SELECT CURDATE() - INTERVAL 3 DAY;" | sed -n '2p'` 
...
all_table_names=(`mysql -u $db_user  -e "USE maindb;SHOW TABLES WHERE Tables_in_maindb LIKE  'a2_logs_%' OR tables_in_maindb LIKE 'a3_logs_%'  OR tables_in_maindb LIKE 'a4_logs_%'  OR tables_in_maindb LIKE 'a5_logs_%' ;" | grep -v maindb`)
...
do
    ...
    if [ $three_days_ago -gt $date_section_in_table_name ]; then
        ...
        mysql -u $db_user -e "use $db_name;DROP TABLE IF EXISTS $single_table_name;"
